...
const MyProjects = () => {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            if (!userDetails?.userDetails?.stud_id) return; // Wait until stud_id is available

            try {
                const token = Cookies.get("token");
                const stud_id = userDetails.userDetails.stud_id;

                console.log("Fetching projects for student ID:", stud_id);
